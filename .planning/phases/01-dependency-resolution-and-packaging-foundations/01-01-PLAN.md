...
**RoMa status:** The current pyproject.toml lists `romav2` directly. The dataclasses>=0.8 metadata bug may or may not cause install failures on Python 3.10+. Testing needed but can proceed in parallel.
  ...
  <action>Present dependency strategy options to user and wait for decision. Test RoMa v2 install in parallel: run `pip install --dry-run romav2` to check if dataclasses metadata bug causes failure on current Python version.</action>
