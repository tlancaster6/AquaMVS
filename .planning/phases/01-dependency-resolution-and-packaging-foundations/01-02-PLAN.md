...
@.planning/phases/01-dependency-resolution-and-packaging-foundations/01-01-SUMMARY.md
@pyproject.toml
</context>
...
4. **Steps:**
   ...
   - Install: `pip install --upgrade pip setuptools wheel && pip install -e ".[dev]"`
   - Handle LightGlue dependency per chosen strategy from Plan 01:
     - If vendored: no extra step needed
     - If prerequisite: add step to `pip install git+https://github.com/cvg/LightGlue.git@edb2b83`
   - Handle AquaCal dependency per chosen strategy:
     - If on PyPI: no extra step needed
     - If prerequisite: add step to `pip install git+https://github.com/tlancaster6/AquaCal.git`
   - Handle RoMa if workaround needed: `pip install --no-deps romav2 && pip install einops>=0.8.1 rich>=14.2.0 tqdm>=4.67.1`
...
Check that matrix includes both OS and all 3 Python versions.
  ...
  <done>
test.yml exists with correct matrix (2 OS x 3 Python), triggers on push/PR to main, installs dependencies including prerequisite packages, runs pytest with coverage.
  </done>
</task>
...
5. Dependency installation steps match strategy chosen in Plan 01
</verification>

<success_criteria>
- CI test workflow runs tests across 6 matrix combinations (2 OS x 3 Python)
