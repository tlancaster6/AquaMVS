...
must_haves:
  truths:
    ...
    - "Smoke test validates notebook code cells parse as valid Python"
  ...
  artifacts:
    ...
    - path: "tests/test_notebook_smoke.py"
      provides: "Import-only smoke test for notebook syntax"
    - path: "docs/index.rst"
...
<task type="auto">
  ...
  **Commit with outputs CLEARED** — notebook will be executed on real hardware before RTD deploy.
...
<task type="auto">
  ...
  Validates that all code cells in documentation notebooks parse as valid Python.
  ...
  Notes:
  - Uses `ast.parse()` only — no execution, no imports actually run
  ...
  - No extra dependencies needed (ast, json, pathlib are stdlib; pytest already in dev deps)
  </action>
  <verify>
  - `pytest tests/test_notebook_smoke.py -v` passes (all notebook cells parse)
  ...
  <done>Smoke test validates all notebook code cells parse as valid Python. Test runs in existing CI without dataset or GPU.</done>
</task>
