...
must_haves:
  truths:
    ...
    - "RTD build installs myst-nb via pip install -e .[dev] in .readthedocs.yaml"
  artifacts:
    - path: "docs/conf.py"
...
<task type="auto">
  ...
  <files>docs/conf.py, pyproject.toml, .readthedocs.yaml, docs/troubleshooting.rst</files>
  <action>
  **docs/conf.py:**
  ...
  **.readthedocs.yaml:**
  - Verify that the existing `pip install -e ".[dev]"` command will install myst-nb (it will, since myst-nb is being added to dev deps above).
  - No changes needed if the current build.commands already include `pip install -e ".[dev]"`. Just confirm this is the case and leave the file unchanged.
  - If the build commands do NOT install dev extras, update to include `pip install -e ".[dev]"` so myst-nb is available during RTD build.
  ...
  **docs/troubleshooting.rst:**
  ...
  - `grep "dev" .readthedocs.yaml` confirms dev extras are installed (ensuring myst-nb available on RTD)
  </verify>
