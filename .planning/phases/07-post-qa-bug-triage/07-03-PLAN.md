...
must_haves:
  truths:
    ...
    - "Each pathway row shows per-stage wall time (undistortion, matching, depth, fusion, surface), total time, point count, cloud density (pts/m²), and outlier removal %"
  ...
  artifacts:
    ...
    - path: "src/aquamvs/benchmark/metrics.py"
      provides: "Relative metric computation (point count, cloud density, outlier %)"
...
- `src/aquamvs/benchmark/synthetic_benchmark.py`
- `src/aquamvs/profiling/synthetic_profile.py`
- `.benchmarks/run_benchmark.py` (standalone script using old torch.profiler approach — superseded by new CLI)
- `.benchmarks/run_profile.py` (standalone profiling script — superseded by new CLI)
...
class PathwayResult:
    ...
    point_count: int = 0
    cloud_density: float = 0.0  # points per m² (bounding box XY area proxy)
...
class BenchmarkResult:
    ...
    config_path: str = ""
    output_dir: str = ""  # Populated from config.output_dir in run_benchmark
    frame: int = 0
...
def run_benchmark(
    ...
    2. Create a FRESH PipelineProfiler() instance for this pathway
       (do NOT reuse a single profiler — profiler.snapshots is a plain
       dict, so reusing overwrites same-named stage keys across pathways)
    3. Set active profiler via set_active_profiler(fresh_profiler)
    4. Read frame, build context, run process_frame
    5. Collect profiler report + relative metrics
    6. Deactivate profiler
    ...
    IMPORTANT: Instantiate PipelineProfiler() inside the pathway loop,
    not outside it. Each pathway gets an isolated snapshots dict.

    IMPORTANT: After loading config via PipelineConfig.from_yaml(config_path),
    set result.output_dir = str(config.output_dir) on the BenchmarkResult before
    returning. The CLI's benchmark_command uses result.output_dir to know where
    to save the markdown report — it must not be left as the empty string default.
    """
    ...
```
...
def compute_relative_metrics(pathway_output_dir: Path) -> dict[str, float]:
    ...
    - cloud_density: points per m² (bounding box XY area as scan area proxy, 0 if no cloud)
    - outlier_removal_pct: percentage of points removed as outliers
    """
    ...
```

Look for `fused_points.ply` in the pathway output dir. Use Open3D to load and count points.

For cloud density (points/m²): compute the bounding box XY extent of the fused point cloud (`pcd.get_axis_aligned_bounding_box()`, then `max_bound[:2] - min_bound[:2]` for XY width/height), multiply to get scan area, then `point_count / scan_area`. This is a proxy — not true surface area, but gives a useful relative comparison across pathways. Report 0 if no cloud exists.

For outlier %, look for the raw cloud before outlier removal vs after, or parse from the fusion stage log. Keep it simple — if data isn't available, report 0 or N/A.
...
def format_console_table(result: BenchmarkResult) -> str:
    ...
    Columns: Pathway | Undist (s) | Match (s) | Depth (s) | Fusion (s) | Surface (s) | Total (s) | Points | Density (pts/m²) | Outlier %
